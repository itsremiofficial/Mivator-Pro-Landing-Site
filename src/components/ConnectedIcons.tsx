...
const icons = {
  IconFast: React.lazy(() => import('@icon/Features/IconFast')),
  IconModify: React.lazy(() => import('@icon/Features/IconModify')),
  IconCompatible: React.lazy(() => import('@icon/Features/IconCompatible')),
  IconUi: React.lazy(() => import('@icon/Features/IconUi')),
  IconFeatures: React.lazy(() => import('@icon/Features/IconFeatures')),
  IconInput: React.lazy(() => import('@icon/Features/IconInput')),
};
...
const ConnectedIcons: React.FC = () => {
  const svgRef = useRef<SVGSVGElement>(null);
  const refShadow = useRef<SVGPathElement>(null);
  const svgSection = useRef<HTMLDivElement>(null);

  // Memoize theme-based color selection
  const storedTheme = useMemo(() => localStorage.getItem('colorScheme') || 'default', []);

  // Optimize gradient configurations
  ...
  useEffect(() => {
    ...
    if (!svg || !section || !shadow) return;

    const animationCleanup = gradientConfigs.map(({ id }) => {
  ...
  return (
    <div ref={svgSection} className="h-screen flex justify-center items-center px-4 iconscontainer">
      <div className="relative w-screen h-[850px] max-w-5xl z-[1] flex flex-col justify-between">
        ...
        <svg ref={svgRef} viewBox="0 0 750 800" className="connected-feature absolute top-0 z-[0] left-1/2 transform -translate-x-1/2 pointer-events-none w-[830px]">
          {/* Previous SVG defs and paths remain unchanged */}
        </svg>{' '}
        <svg ref={svgRef} viewBox="0 0 750 800" className="connected-feature absolute top-0 z-[0] left-1/2 transform -translate-x-1/2 pointer-events-none w-[830px]">
          {' '}
          <defs>
...
export default memo(ConnectedIcons);
