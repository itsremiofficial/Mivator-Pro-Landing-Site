...
import { useDispatch } from 'react-redux';
import { useEffect, useRef, useState } from 'react';
import { toggleTheme, toggleColorScheme } from '../store/themeConfigSlice';
import IconClose from '../components/Icon/IconClose';
...
const ThemeCustomizer = () => {
  ...
  const handleThemeChange = (themeKey: keyof typeof colorSchemes) => {
    if (themeKey === 'mivatorsilver') {
      dispatch(toggleTheme('light'));
    } else {
      dispatch(toggleTheme('dark'));
  ...
  return (
    <div>
      <button type="button" onClick={() => setShowCustomizer(!showCustomizer)} className="theme-toggler-btn">
        Toggle Theme Customizer
      </button>

      {showCustomizer && (
        <div ref={drawerRef} className="theme-customizer-drawer">
          ...
          <div className="theme-options">
            {Object.keys(colorSchemes).map((themeKey) => (
              <div
                ...
                style={{ backgroundColor: colorSchemes[themeKey as keyof typeof colorSchemes].color }}
                onClick={() => handleThemeChange(themeKey as keyof typeof colorSchemes)}
              >
                {colorSchemes[themeKey as keyof typeof colorSchemes].name}
              </div>
            ))}
          </div>
        </div>
