    ...
    <div className="p-4 h-full relative flex items-center justify-center">
      <div className="flex flex-row flex-shrink-0 justify-center items-center gap-2">
        ...
        <Container className="h-20 w-20 circle-2">
          <IconCopy className="size-6 dark:text-white" />
        </Container>
        <Container className="w-24 h-24 circle-3">
      ...
      <div className="h-32 w-px absolute inset-0 m-auto z-[-1] bg-gradient-to-b from-transparent via-primary-100 to-transparent animate-move">
        <div className="w-10 h-32 top-1/2 -translate-y-1/2 absolute -left-10">
          <Sparkles />
        </div>
      </div>
      <div className="horizontal-gradient absolute inset-0"></div>
    </div>
  );
};
const Sparkles = () => {
...
export const CardSkeletonContainer = ({ className, children }: { className?: string; children: React.ReactNode }) => {
  return <div className={`z-50 ${className}`}>{children}</div>;
};

const Container = ({ className, children }: { className?: string; children: React.ReactNode }) => {
  return (
    <div
      className={`size-20 rounded-full flex items-center justify-center bg-[rgba(248,248,248,0.01)]
